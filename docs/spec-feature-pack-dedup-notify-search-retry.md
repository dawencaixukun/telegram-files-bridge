...
#### 2) 直投与下载接口防重联动：
- `POST /api/tg/resolve-link`：返回的媒体文件数组中，每个对象统一注入 `dedup` 字段：
  ```json
  "dedup": {
    "isDuplicate": true,
    "type": "cloud",
    "prompt": "已归档至网盘",
    "cloudPath": "/AliyunDrive/Media/..."
  }
  ```
...
### 5.3 接口契约规范：`GET /api/search/aggregate`
...
#### 2) 一键批量重试接口：`POST /api/archive/batch-retry`
- **请求体（JSON）**：
```json
{
  "category": "NETWORK_TIMEOUT",
